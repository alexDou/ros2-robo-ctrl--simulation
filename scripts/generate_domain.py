...
def emit_typescript(ir: DomainIR) -> str:
    ...
    has_ping = any("PING" in e.variants for e in ir.enums)
    if has_ping:
        lines.append("")
        lines.append("export function createPingCommand(params?: {")
        lines.append("  senderId?: string;")
        lines.append("  commandId?: string;")
        lines.append("  timestampNs?: bigint | number;")
        lines.append("}): RobotCommand {")
        lines.append("  return {")
        lines.append("    command_id: params?.commandId ?? (typeof crypto !== 'undefined' && crypto.randomUUID ? crypto.randomUUID() : `cmd-${Date.now()}`),")
        lines.append("    sender_id: params?.senderId ?? 'teleop-ui',")
        lines.append("    timestamp_ns: params?.timestampNs ?? BigInt(Date.now()) * 1_000_000n,")
        lines.append("    type: CommandType.PING,")
        lines.append("    payload: {},")
        lines.append("  };")
        lines.append("}")

    lines.append("")
    lines.append("function parseJsonIfNeeded(input: string | unknown): unknown {")
    lines.append("  if (typeof input === 'string') {")
    lines.append("    return JSON.parse(input);")
    lines.append("  }")
    lines.append("  return input;")
    lines.append("}")
    lines.append("")
    lines.append("function parseTimestampNs(val: unknown): string | number | bigint {")
    lines.append("  if (val === undefined || val === null) {")
    lines.append('    throw new Error("Missing required field \'timestamp_ns\'");')
    lines.append("  }")
    lines.append("  let ts: bigint;")
    lines.append("  try {")
    lines.append("    ts = typeof val === 'bigint'")
    lines.append("      ? val")
    lines.append("      : BigInt(typeof val === 'number' ? Math.floor(val) : String(val));")
    lines.append("  } catch {")
    lines.append('    throw new Error("Field \'timestamp_ns\' must be a valid integer");')
    lines.append("  }")
    lines.append("  if (ts < 0n) {")
    lines.append('    throw new Error("Field \'timestamp_ns\' must be a non-negative integer");')
    lines.append("  }")
    lines.append("  return val as string | number | bigint;")
    lines.append("}")
    ...
    for m in ir.models:
        if m.is_submodel:
            continue
        lines.append("")
        lines.append(f"export function parse{m.name}(input: string | unknown): {m.name} {{")
        lines.append("  const data = parseJsonIfNeeded(input);")
        lines.append("  if (!data || typeof data !== 'object') {")
        lines.append(f"    throw new Error('{m.name} payload must be an object');")
        lines.append("  }")
        lines.append("")
        lines.append("  const obj = data as Record<string, unknown>;")

        for f in m.fields:
            if f.kind == "literal":
                lines.append(f"  if (obj.{f.name} !== '{f.ref}') {{")
                lines.append(f"    throw new Error(`Expected frame type '{f.ref}', got '${{String(obj.{f.name})}}'`);")
                lines.append("  }")
                lines.append(f"  if (typeof obj.{f.name} !== 'string' || !obj.{f.name}) {{")
                lines.append(f'    throw new Error("Missing required field \'{f.name}\'");')
                lines.append("  }")
            elif f.name == "timestamp_ns" and f.kind == "int":
                lines.append(f"  const {f.name} = parseTimestampNs(obj.{f.name});")
            elif f.kind == "string":
                if f.required:
                    lines.append(f"  if (typeof obj.{f.name} !== 'string' || !obj.{f.name}) {{")
                    lines.append(f'    throw new Error("Missing required field \'{f.name}\'");')
                ...
                desc_label = to_snake_case(f.ref).replace("_", " ")
                lines.append(f"  const valid{f.ref} = Object.values({f.ref}) as string[];")
                lines.append(f"  if (typeof obj.{f.name} !== 'string' || !valid{f.ref}.includes(obj.{f.name})) {{")
                lines.append(f"    throw new Error(`Invalid {desc_label}: ${{String(obj.{f.name})}}`);")
                lines.append("  }")
            elif f.kind == "fixed_array":
                lines.append(f"  if (!Array.isArray(obj.{f.name}) || obj.{f.name}.length !== {f.minimum}) {{")
                lines.append("    throw new Error(")
                lines.append(f"      `{m.name} must contain exactly {f.minimum} joint positions, received ${{Array.isArray(obj.{f.name}) ? obj.{f.name}.length : 'non-array'}}`")
                lines.append("    );")
                lines.append("  }")
                lines.append("")
                lines.append(f"  for (let i = 0; i < {f.minimum}; i++) {{")
                lines.append(f"    if (typeof obj.{f.name}[i] !== 'number' || !Number.isFinite(obj.{f.name}[i])) {{")
                lines.append(f"      throw new Error(`Joint position at index ${{i}} is not a valid finite number`);")
                lines.append("    }")
                lines.append("  }")
            elif f.kind == "generic_map":
                lines.append(f"  if (obj.{f.name} === undefined || obj.{f.name} === null || typeof obj.{f.name} !== 'object' || Array.isArray(obj.{f.name})) {{")
                lines.append(f'    throw new Error("Missing or invalid \'{f.name}\' object");')
                lines.append("  }")
            elif f.kind == "model":
                sub_model = next((sm for sm in ir.models if sm.name == f.ref), None)
                if sub_model:
                    lines.append(f"  let {f.name}: {f.ref} | undefined;")
                    lines.append(f"  if (obj.{f.name} && typeof obj.{f.name} === 'object') {{")
                    lines.append(f"    const subObj = obj.{f.name} as Record<string, unknown>;")
                    conds = []
                    for sf in sub_model.fields:
                        if sf.kind == "float":
                            conds.append(f"typeof subObj.{sf.name} === 'number'")
                        elif sf.kind == "string":
                            conds.append(f"typeof subObj.{sf.name} === 'string'")
                        elif sf.kind == "int":
                            conds.append(f"typeof subObj.{sf.name} === 'number'")
                    lines.append(f"    if ({' && '.join(conds)}) {{")
                    lines.append(f"      {f.name} = {{")
                    for sf in sub_model.fields:
                        lines.append(f"        {sf.name}: subObj.{sf.name} as any,")
                    lines.append("      };")
                    lines.append("    }")
                    lines.append("  }")
        ...
        lines.append("  return {")
        for f in m.fields:
            if f.name == "timestamp_ns" and f.kind == "int":
                lines.append("    timestamp_ns,")
            elif f.kind == "fixed_array":
                lines.append(f"    {f.name}: obj.{f.name} as {f.ref},")
            elif f.kind == "model":
                lines.append(f"    {f.name},")
            elif f.kind == "literal":
                lines.append(f"    {f.name}: '{f.ref}',")
                ...
                lines.append(f"    {f.name}: obj.{f.name} as Record<string, unknown>,")
            elif not f.required and f.kind == "string":
                lines.append(f"    {f.name}: typeof obj.{f.name} === 'string' ? obj.{f.name} : undefined,")
            else:
                lines.append(f"    {f.name}: obj.{f.name} as any,")
        lines.append("  };")
        lines.append("}")
        ...
        lines.append("  try {")
        lines.append(f"    parse{m.name}(input);")
    ...
    if ir.channels:
        lines.append("")
        lines.append("function validateRobotId(robotId: string): void {")
        lines.append("  if (!robotId || robotId.includes('/') || robotId.includes('\\\\') || robotId.includes(' ')) {")
        lines.append("    throw new Error(`Invalid robot ID '${robotId}': must be non-empty and not contain slashes or whitespace`);")
        lines.append("  }")
        lines.append("}")
        ...
        for ch in ir.channels:
            ...
            lines.append("  validateRobotId(robotId);")
            lines.append(f"  return `robot/${{robotId}}/{ch}`;")
            lines.append("}")

        lines.append("")
        ch_union = " | ".join(f"'{c}'" for c in ir.channels)
        lines.append("export function parseRobotTopic(")
        lines.append("  topic: string")
        lines.append(f"): {{ robotId: string; channel: {ch_union} }} | null {{")
        lines.append("  const parts = topic.split('/');")
        lines.append("  if (parts.length !== 3 || parts[0] !== 'robot' || !parts[1]) {")
        lines.append("    return null;")
        lines.append("  }")
        valid_ch_checks = " && ".join(f"parts[2] !== '{c}'" for c in ir.channels)
        lines.append(f"  if ({valid_ch_checks}) {{")
        lines.append("    return null;")
        lines.append("  }")
        lines.append("  return {")
        lines.append("    robotId: parts[1],")
        lines.append(f"    channel: parts[2] as {ch_union},")
        lines.append("  };")
        lines.append("}")
