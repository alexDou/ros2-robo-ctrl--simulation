...
Instead of using messy legacy virtual environments, we will use uv to initialize a modern, deterministic Python workspace.
   ...
   (Note: We will not add rclpy via uv because ROS2 Jazzy packages are tied natively to the system Python environment inside our VirtualBox Ubuntu container. uv will be configured to inherit system packages).
   4. Write a quick unit test using pytest to assert our domain command serialization logic before writing any pipeline code (TDD).
