...
updated_at: 2026-09-12T15:23:00Z
...
- [x] Incoming 30 Hz `ArmJointPositions` stored in mutable non-reactive buffer reference without VDOM re-rendering.
- [x] Animation loop synchronizes all 6 canonical revolute joints by exact joint name using `URDFRobot.setJointValue`.
- [x] Dirty-checked render loop skips WebGL draw calls when joint angles and camera position remain unchanged.
- [x] Manipulator root group applies REP-103 coordinate alignment (`robotGroup.rotation.x = -Math.PI / 2`) with zero manual component swizzling.
- [x] Link world matrices update accurately in response to dynamic `ArmJointPositions` updates.
- [x] Vitest unit tests verify kinematic transformation matrix propagation, REP-103 alignment, and idle render bypass.
