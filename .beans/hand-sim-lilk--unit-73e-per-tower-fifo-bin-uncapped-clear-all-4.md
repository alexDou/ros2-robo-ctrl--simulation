---
# hand-sim-lilk
title: 'Unit 7.3e: Per-tower FIFO + bin uncapped + Clear all-4'
status: todo
type: task
priority: normal
tags:
...
As an operator, I want each tower to evict its oldest bottom Gearwheel when an 11th arrives (counter stays at 10), the bin to pile unbounded, and ClearWorkspace to wipe all four destinations and reset every counter and the bin icon, so that long runs stay bounded and I can always restart cleanly.
