...
## Corpus Check
- 205 files · ~122,863 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 1910 nodes · 2471 edges · 190 communities (140 shown, 30 thin omitted)
- Extraction: 93% EXTRACTED · 7% INFERRED · 0% AMBIGUOUS · INFERRED: 164 edges (avg confidence: 0.94)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `d6c0a280`
...
1. `TeleopPage` - 53 edges
2. `EdgeNode` - 47 edges
3. `UR5eKinematics` - 29 edges
4. `CommandType` - 23 edges
5. `RobotCommand` - 23 edges
6. `RobotState` - 21 edges
7. `MockMotionPublisher` - 21 edges
8. `TelemetrySubscription` - 18 edges
9. `compilerOptions` - 18 edges
10. `ServiceHarness` - 17 edges
...
- `test_workcell_state_non_finite_rejection_and_frozen_dataclass()` --uses--> `WorkcellState`  [INFERRED]
...
## Communities (190 total, 30 thin omitted)
...
Cohesion: 0.18
Nodes (24): CommandType, ErrorFrame, isErrorFrame(), PalmAction, PickAndPlaceTargetPayload, RobotCommand, RobotTelemetryEvent, serializeCommand() (+16 more)
...
Cohesion: 0.17
Nodes (11): ErrorFrame, Node, PalmAction, EdgeNode, Any, RobotTelemetryEvent, Ingests a JointState ROS2 message and updates canonical joint positions. Args:…, Emits a periodic 30 Hz RobotTelemetryEvent with current joint positions. (+3 more)

### Community 97 - "BaseModel"
Cohesion: 0.12
Nodes (18): BaseModel, ClearWorkspacePayload, EmergencyStopPayload, InferenceMetrics, PalmActuatePayload, PalmState, PickAndPlaceTargetPayload, ResetFaultPayload (+10 more)
...
### Community 141 - "items"
Cohesion: 0.11
Nodes (20): HOME, INSPECT_POSE, READY, description, items, maxItems, minItems, title (+12 more)
...
### Community 144 - "edge_node/__init__.py"
Cohesion: 0.13
Nodes (14): SpawnObjectPayload, SpawnObjectType, main(), CLI Entrypoint for EdgeNode process., JointState extraction and mapping for canonical 6-DoF UR5e arm., EdgeNode ROS2 node and Zenoh DataFabric command ingestion service., ActiveGear, Exception (+6 more)
...
### Community 158 - "domain.py"
Cohesion: 0.13
Nodes (21): Enum, PalmAction, parse_robot_topic(), PoseName, robot_command_topic(), robot_telemetry_topic(), SpawnObjectType, TrajectoryExecutePayload (+13 more)
...
### Community 177 - "robot_command.schema.json"
Cohesion: 0.25
...
### Community 180 - "$defs"
Cohesion: 0.33
Nodes (6): $defs, trajectory_execute_payload, additionalProperties, description, title, type

### Community 181 - "kinematics.py"
Cohesion: 0.12
Nodes (15): KinematicsError, KinematicSingularityError, normalize_angle(), OutOfReachError, Exception, Analytical UR5e inverse kinematics solver and waypoint planner. Implements…, Validates that Cartesian coordinates fall within the robot's physical reach., Calculates Euclidean angular distance avoiding multi-revolution flips. (+7 more)

### Community 182 - "MockJointStatePublisher"
Cohesion: 0.19
...
### Community 183 - "WorkcellState"
Cohesion: 0.18
Nodes (8): SpawnObjectPayload, Typed payload for SPAWN_OBJECT command, In-process workcell state tracker recording active workpiece presence and…, Returns True if an active gear is currently present in the workcell., Returns (x, y, z) coordinates of the active gear, or None., Resets active gear presence and coordinates., WorkcellState, test_workcell_state_domain_unit()

### Community 184 - ".solve_ik_matrix"
Cohesion: 0.18
...
## Knowledge Gaps
- **889 isolated node(s):** `rust_feedback.sh script`, `ros2-robot-controller-simulation`, `$schema`, `$id`, `title` (+884 more)
...
- **Why does `$defs` connect `$defs` to `emergency_stop_payload`, `reset_fault_payload`, `spawn_object_payload`, `robot_command.schema.json`, `palm_actuate_payload`, `pick_and_place_target_payload`, `clear_workspace_payload`?**
  _High betweenness centrality (0.030) - this node is a cross-community bridge._
- **Why does `EdgeNode` connect `EdgeNode` to `BaseModel`, `JointStateMapper`, `edge_node/__init__.py`, `test_edge_node.py`, `MockJointStatePublisher`, `WorkcellState`, `domain.py`?**
  _High betweenness centrality (0.028) - this node is a cross-community bridge._
- **Why does `properties` connect `enum` to `robot_command.schema.json`?**
  _High betweenness centrality (0.017) - this node is a cross-community bridge._
- **Are the 30 inferred relationships involving `EdgeNode` (e.g. with `ClearWorkspacePayload` and `CommandType`) actually correct?**
  _`EdgeNode` has 30 INFERRED edges - model-reasoned connections that need verification._
...
- **Are the 19 inferred relationships involving `CommandType` (e.g. with `EdgeNode` and `test_clear_workspace_payload_serialization()`) actually correct?**
  _`CommandType` has 19 INFERRED edges - model-reasoned connections that need verification._
- **Are the 20 inferred relationships involving `RobotCommand` (e.g. with `EdgeNode` and `test_clear_workspace_payload_serialization()`) actually correct?**
  _`RobotCommand` has 20 INFERRED edges - model-reasoned connections that need verification._