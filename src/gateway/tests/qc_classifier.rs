...
//!
//! Random stub: uniform WHITE/GREEN/BLUE, ~20% defective.
...
#[test]
fn random_stub_defective_rate_near_20_percent() {
    let draws = classify_n(&mut RandomQcClassifier::seeded(42), 2000);
    let defective = draws.iter().filter(|(_, intact)| !intact).count();
    assert!(
        (320..=480).contains(&defective),
        "defective count {defective} outside 320..=480 (16-24%)"
    );
}

#[test]
fn enrich_writes_color_and_defective_keys() {
    let blind =
    ...
    assert_eq!(enriched["defective"], serde_json::Value::Bool(true));
