use gateway::domain::{
    parse_robot_topic, robot_command_topic, robot_telemetry_topic, CommandType, ErrorFrame,
    InferenceMetrics, RobotCommand, RobotState, RobotTelemetryEvent,
};
use serde_json::json;
...
fn test_robot_telemetry_event_malformed_fails() {
    ...
    assert!(result.is_err(), "Expected deserialization to fail for invalid joint count");
